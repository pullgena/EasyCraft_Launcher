...
function homePage(){return page(`<div class="brand">EASYCRAFT LOGIN · BETA 5</div><section class="card"><h1>Microsoft 계정으로 로그인</h1><p>로그인을 완료하면 EasyCraft Launcher에 입력할 일회용 코드가 발급됩니다.</p><p class="steps">먼저 학교 PC에서 시도해 보고, Microsoft 로그인이 막히면 이 사이트를 휴대폰에서 열어 진행하세요.</p><a class="btn" href="/oauth/start">Microsoft로 로그인</a></section>`);}
...
export default {
  async fetch(request,env){
    ...
    try{
      if(url.pathname==='/health')return json({ok:true,service:'easycraft-auth',version:'beta.5'});
