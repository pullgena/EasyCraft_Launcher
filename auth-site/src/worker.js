...
async function postForm(url,values){ const r=await fetch(url,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded','accept':'application/json'},body:new URLSearchParams(values)});const d=await r.json().catch(()=>({}));if(!r.ok||d.error)throw new Error(d.error_description||d.error||`Microsoft OAuth HTTP ${r.status}`);return d; }
async function postJson(url,body){ const r=await fetch(url,{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify(body)});const d=await r.json().catch(()=>({}));if(!r.ok||d.error)throw new Error(d.error_description||d.errorMessage||d.error||`HTTP ${r.status}`);return d; }
async function getJson(url,headers={}){ const r=await fetch(url,{headers:{'accept':'application/json',...headers}});const d=await r.json().catch(()=>({}));if(!r.ok||d.error)throw new Error(d.errorMessage||d.error||`HTTP ${r.status}`);return d; }
...
function homePage(){return page(`<div class="brand">EASYCRAFT LOGIN · BETA 6</div><section class="card"><h1>Microsoft 계정으로 로그인</h1><p>로그인을 완료하면 EasyCraft Launcher에 입력할 일회용 코드가 발급됩니다.</p><p class="steps">먼저 학교 PC에서 시도해 보고, Microsoft 로그인이 막히면 이 사이트를 휴대폰에서 열어 진행하세요.</p><a class="btn" href="/oauth/start">Microsoft로 로그인</a></section>`);}
...
export default {
  async fetch(request,env){
    ...
    try{
      if(url.pathname==='/health')return json({ok:true,service:'easycraft-auth',version:'beta.6'});
