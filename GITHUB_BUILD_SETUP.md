# EasyCraft Launcher v0.4.22 GitHub Release 빌드

정식 버전은 **Release 태그가 정확히 `v0.4.22`일 때만** Windows 빌드가 실행됩니다. `v`와 `0`은 붙여서 입력하세요.
...
4. 태그를 정확히 `v0.4.22`로 생성합니다.
5. `RELEASE_NOTES.md` 내용을 Release 설명에 붙여넣습니다.
6. Release를 Publish합니다.
7. `Release v0.4.22 Windows EXE + Auto Update` Workflow가 자동으로 실행됩니다.
8. 성공하면 같은 Release Assets에 다음 파일이 등록됩니다.

```text
EasyCraft-Launcher-Setup-0.4.22.exe
EasyCraft-Launcher-Setup-0.4.22.exe.blockmap
latest.yml
```
