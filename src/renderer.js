...
function switchView(view) {
  ...
  const copy = {
    home:['EASYCRAFT','오늘은 어떤 Minecraft를 플레이할까요?'],
    content:['CONTENT','모드와 콘텐츠를 런처 안에서 바로 설치하세요'],
...
async function renderContent() {
  ...
  renderShaderVisibility();
  if(!inst){$('#searchResults').innerHTML='<div class="empty">인스턴스를 선택하면 콘텐츠를 검색할 수 있습니다.</div>';$('#installedList').innerHTML='<div class="empty">인스턴스가 선택되지 않았습니다.</div>';return;}
...
function renderSearchResults(){ const area=$('#searchResults');area.innerHTML='';if(!state.searchResults.length){area.innerHTML='<div class="empty">검색 결과가 없습니다.</div>';return;}for(const item of state.searchResults){const row=document.createElement('div');row.className='result-item';row.innerHTML=`${item.iconUrl?`<img class="result-icon" src="${esc(item.iconUrl)}" alt="">`:'<div class="result-placeholder">◇</div>'}<div class="item-copy"><strong>${esc(item.title)}</strong><span>${esc(item.author||'')} · ${Number(item.downloads||0).toLocaleString()} 다운로드</span><span>${esc(item.description||'')}</span></div><button class="btn ${item.installed?'subtle':'primary'} small install" ${item.installed?'disabled':''}>${item.installed?'설치됨':'설치'}</button>`;row.querySelector('.install')?.addEventListener('click',async e=>{if(item.installed)return;const btn=e.currentTarget;btn.disabled=true;btn.textContent='확인 중…';const plan=await api.modrinthInstallPlan(currentInstance().id,item.projectId);if(!plan.ok){toast(plan.error||'설치 정보를 확인하지 못했습니다.',true);btn.disabled=false;btn.textContent='설치';return;}let allow=false;if(plan.dependencies?.length){const names=plan.dependencies.map(d=>d.title).join(', ');allow=confirm(`${plan.rootTitle}를 설치하려면 ${names}가 필요해요. 설치할까요?`);if(!allow){btn.disabled=false;btn.textContent='설치';return;}}btn.textContent='설치 중…';const r=await api.modrinthInstall(currentInstance().id,item.projectId,allow);if(!r.ok){toast(r.error||'설치하지 못했습니다.',true);btn.disabled=false;btn.textContent='설치';return;}toast(`${r.title||item.title} 설치 완료`);item.installed=true;await refreshCapabilities();await renderContent();renderSearchResults();});area.appendChild(row);}}
...
function updateSettingsText(u=state.update){ const version=state.appVersion; const title=$('#updateStatusTitle'), text=$('#updateStatusText'), action=$('#settingsUpdateActionBtn'), progress=$('#updateProgress'); progress.style.width=`${u.percent||0}%`; action.classList.add('hidden'); action.dataset.action=''; if(u.state==='latest'){title.textContent='최신 버전입니다';text.textContent=`EasyCraft v${version}을 사용하고 있습니다.`;}else if(u.state==='available'){title.textContent=`v${u.availableVersion} 업데이트 가능`;text.textContent='새 버전을 다운로드할 수 있습니다.';action.textContent='업데이트';action.dataset.action='download';action.classList.remove('hidden');}else if(u.state==='downloading'){title.textContent=`업데이트 다운로드 중 · ${u.percent||0}%`;text.textContent='다운로드가 끝나면 재시작해서 적용할 수 있습니다.';}else if(u.state==='downloaded'){title.textContent=`v${u.availableVersion} 준비 완료`;text.textContent='재시작하면 새 버전이 적용됩니다.';action.textContent='재시작하여 업데이트';action.dataset.action='install';action.classList.remove('hidden');}else if(u.state==='checking'||u.state==='idle'){title.textContent='업데이트 확인 중';text.textContent='최신 버전을 확인하고 있습니다.';}else if(u.state==='dev'){title.textContent='개발 모드';text.textContent='설치된 EXE에서 업데이트를 확인할 수 있습니다.';}else if(u.state==='error'){title.textContent='업데이트 확인 오류';text.textContent=u.error||'업데이트 서버에 연결하지 못했습니다.';}else{title.textContent='업데이트 상태';text.textContent='업데이트 확인 버튼을 눌러 확인할 수 있습니다.';} }
function renderStartupUpdate(u=state.update){ const gate=$('#startupGate'), checking=$('#gateChecking'), avail=$('#gateAvailable'); if(state.updatePromptDismissed){gate.classList.add('hidden');return;} if(u.state==='checking'||u.state==='idle'){gate.classList.remove('hidden');checking.classList.remove('hidden');avail.classList.add('hidden');return;} if(u.state==='available'||u.state==='downloading'||u.state==='downloaded'){gate.classList.remove('hidden');checking.classList.add('hidden');avail.classList.remove('hidden');$('#gateUpdateTitle').textContent=u.state==='downloaded'?`EasyCraft v${u.availableVersion} 준비 완료`:`EasyCraft v${u.availableVersion} 업데이트`;$('#gateUpdateDescription').textContent=u.state==='downloaded'?'재시작하면 새 버전을 바로 사용할 수 있습니다.':u.state==='downloading'?`업데이트를 다운로드하고 있습니다. ${u.percent||0}%`:`현재 v${state.appVersion} → 새 버전 v${u.availableVersion}. 지금 업데이트하시겠어요?`;$('#gateProgressWrap').classList.toggle('hidden',u.state==='available');$('#gateProgress').style.width=`${u.percent||0}%`;$('#updateLaterBtn').disabled=u.state==='downloading';$('#updateNowBtn').disabled=u.state==='downloading';$('#updateNowBtn').textContent=u.state==='downloaded'?'재시작하여 업데이트':u.state==='downloading'?'다운로드 중…':'업데이트';return;} gate.classList.add('hidden');}
